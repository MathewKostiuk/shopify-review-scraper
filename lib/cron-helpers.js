const { getAllThemes, getReviews, saveReview, deleteReview } = require('./data-helpers');
...
const checkForNewReviews = async () => {
  ...
  const themesData = await Promise.all(themes.map(async theme => {
    ...
    try {
      pageData = await scrapeTheme(theme, 1);
    } catch (error) {
      console.log(error)
      return null;
    }
...
const fetchReviews = async (themes) => {
  const themeReviews = await Promise.all(themes.map(async theme => {
    ...
    for (let i = 1; i <= theme.pagesToFetch; i++) {
      ...
      try {
        pageData = await scrapeTheme(theme, i);
      } catch (error) {
        console.log(error);
        continue;
      }
...
module.exports = { checkForNewReviews, flattenArray }
